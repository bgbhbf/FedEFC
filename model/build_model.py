from model.lenet import LeNet, CNN
...
def build_model(args):
    ...
    if args.model == 'lenet':
        ...
        netglob = CNN(input_channel=1).to(args.device)
...
def initialize_weights(model):
    if isinstance(model, nn.Conv2d) or isinstance(model, nn.Linear):
        nn.init.xavier_uniform_(model.weight)  # Xavier init.
        if model.bias is not None:
            nn.init.constant_(model.bias, 0) #bias init at 0
