...
def non_iid_dirichlet_sampling(y_train, num_classes, p, num_users, seed, alpha_dirichlet=100):
    ...
    while np.min(n_classes_per_client) == 0:
        ...
        Phi[invalid_idx] = np.random.binomial(1, p, size=(len(invalid_idx), num_classes)) #reallocation for cleint which has no classes
        n_classes_per_client = np.sum(Phi, axis=1) #summation as above classes per clients
